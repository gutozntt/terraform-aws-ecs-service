locals {
  ...
  secret_vars             = var.secrets
  secret_vars_keys        = var.map_secrets != null ? keys(var.map_secrets) : [for m in local.secret_vars : lookup(m, "name")]
  secret_vars_values      = var.map_secrets != null ? values(var.map_secrets) : [for m in local.secret_vars : lookup(m, "valueFrom")]
  ...
  final_secret_vars = length(local.sorted_secret_vars) > 0 ? local.sorted_secret_vars : null

  depends_on_for_firelens = [{
  ...
  container_definition = {
    ...
    user                   = var.user
    dependsOn              = var.enable_firelens ? local.depends_on_for_firelens : var.container_depends_on
    ...
    secrets                = local.final_secret_vars
  ...
  firelens_container = {
    essential    = true
    image        = "906394416424.dkr.ecr.${var.region}.amazonaws.com/aws-for-fluent-bit:latest"
    name         = "log_router"
    cpu          = 0
    environment  = []
    mountPoints  = []
  ...
  firelens_container_json_map = jsonencode(local.firelens_container)
}
